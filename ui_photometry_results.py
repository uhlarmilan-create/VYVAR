...
def _normalize_sidecar_dataframe(df: pd.DataFrame, csv_path: Path) -> pd.DataFrame:
    ...
    out["catalog_id_key"] = out["catalog_id"].map(normalize_gaia_source_id) if "catalog_id" in out.columns else ""
    return out
...
def _load_comp_and_targets(
    ...
    try:
        ...
        if cp.is_file():
            comp_df = pd.read_csv(cp, dtype=_GAIA_ID_DTYPE)
    except Exception:  # noqa: BLE001
        comp_df = pd.DataFrame(columns=comp_cols)
    try:
        vp = ps / "variable_targets.csv"
        if vp.is_file():
            var_df = pd.read_csv(vp, dtype=_GAIA_ID_DTYPE)
    except Exception:  # noqa: BLE001
        var_df = pd.DataFrame(columns=var_cols)
    try:
...
def _compute_differential_mag(
    ...
    *,
    ...
    comp_ids = [normalize_gaia_source_id(x) for x in comp_df["catalog_id"].tolist()]
    comp_ids = [c for c in comp_ids if c]
    if not comp_ids:
        return pd.DataFrame(result_rows)

    cdf = comp_df.copy()
    cdf["_mk"] = cdf["catalog_id"].map(normalize_gaia_source_id)
    ...
    tgt_key = normalize_gaia_source_id(target_catalog_id)
    if not tgt_key:
        return pd.DataFrame(result_rows)

    for frame_id, frame in frames_df.groupby(gb_col, sort=False):
        frame = frame.copy()
        frame["_mk"] = frame["catalog_id"].map(normalize_gaia_source_id) if "catalog_id" in frame.columns else ""
        ...
        comps["_mk2"] = comps["catalog_id"].map(normalize_gaia_source_id)
...
def _plot_light_curve(
    ...
    *,
    ...
    _hover_t = f"{x_short}=%{{customdata:.6f}}<br>Magnitude=%{{y:.4f}}<extra></extra>"
...
def _load_per_frame_sidecars(pipeline: Any, draft_id: int | None) -> pd.DataFrame | None:
    if draft_id is None:
        st.info("No draft selected or ARCHIVE_PATH is missing in the database.")
        return None
    try:
    ...
    if not all_csvs:
        st.info(
            "No per-frame catalog CSV files found. "
            "Run **MAKE MASTERSTAR** in the VAR-STREM tab."
        )
        return None
    ...
    for p in paths:
        try:
            raw = pd.read_csv(p, low_memory=False, dtype=_GAIA_ID_DTYPE)
            if raw.empty:
                continue
            ...
            continue
    if not parts:
        st.info("Run per-frame catalog export first.")
        return None
...
def _read_optional_csv(path: Path) -> pd.DataFrame | None:
    if not path.is_file():
        return None
    try:
        return pd.read_csv(path, dtype=_GAIA_ID_DTYPE)
    except Exception:  # noqa: BLE001
        return None
...
def _series_for_star(combined: pd.DataFrame, cid: str, *, mag_col: str, flux_col: str) -> pd.DataFrame:
    sub = combined[combined["catalog_id_key"] == normalize_gaia_source_id(cid)].copy()
    if sub.empty:
        return sub
    xcol = next(
        (
...
def render_aperture_results(pipeline: Any, draft_id: int | None) -> None:
    ...
    if not tgt_labels:
        st.warning("No target stars (variable_targets.csv or VSX in sidecars).")
        return
    ...
    if not clabels:
        st.warning("No comparison stars.")
        return
    sel = st.multiselect(
        "Comparison stars",
        options=clabels,
    ...
    st.markdown("---")
    st.markdown("#### Differential magnitude — light curve")

    comp_df, var_df = _load_comp_and_targets(ap)

    if comp_df.empty:
        st.warning(
            "comparison_stars.csv is not available — "
            "run **MAKE MASTERSTAR** (plate solve + catalog)."
        )
    elif var_df.empty:
        st.info(
            "variable_targets.csv is empty — "
            "add target variable stars to the file."
        )
    else:
        var_work = var_df[var_df["catalog_id"].map(lambda x: bool(normalize_gaia_source_id(x)))].reset_index(drop=True)
        if var_work.empty:
            st.info("variable_targets.csv contains no valid catalog_id for target selection.")
        else:
            _time_options = {
            ...
            _weighted = st.checkbox(
                "Weighted comp ensemble mean (1/σ²)",
                value=True,
            ...
            if lc_df.empty:
                st.warning(
                    f"Star {_var_names[_sel_idx]} was not found "
                    "in per-frame CSV or comparison stars are missing."
                )
            else:
                col1, col2, col3, col4 = st.columns(4)
                col1.metric("Point count", len(lc_df))
                col2.metric("Mean mag", f"{lc_df['cal_mag'].mean():.4f}")
                col3.metric("RMS", f"{lc_df['cal_mag'].std():.4f}")
                col4.metric("Comp stars", int(lc_df["n_comp_used"].median()))

                fig_lc = _plot_light_curve(
                    lc_df,
                    title=f"Light curve — {_var_names[_sel_idx]} (Aperture)",
                    time_col=_time_col,
                )
                st.plotly_chart(fig_lc, width="stretch")

                with st.expander("Zero point stability of comp ensemble", expanded=False):
                    ...
                    fig_zp.add_trace(
                        go.Scatter(
                            ...
                            name="ZP",
                            customdata=zp_raw,
                            hovertemplate="Time=%{customdata:.6f}<extra></extra>",
                        )
                    )
                    fig_zp.add_trace(
                        go.Scatter(
...
def render_psf_results(pipeline: Any, draft_id: int | None) -> None:
    ...
    if not bool(getattr(cfg, "psf_photometry_enabled", False)):
        st.warning(
            "PSF photometry is disabled. Enable it in settings (`config.json` → `psf_photometry_enabled`)."
        )
        return
    ...
    if comp_df.empty:
        ...
        if var_work.empty:
            ...
            if lc_df.empty:
                ...
                fig_lc = _plot_light_curve(
                    lc_df,
                    title=f"Light curve — {_var_names[_sel_idx]} (PSF)",
                    time_col=_time_col,
                )
                st.plotly_chart(fig_lc, width="stretch")

                if st.checkbox(
                    "Compare Aperture vs PSF",
                    ...
                    cmp_hover = "Time=%{customdata:.6f}<extra></extra>"
                    fig_cmp = go.Figure()
                    if not lc_ap.empty:
                    ...
                    fig_cmp.update_layout(
                        title="Aperture vs PSF — differential magnitude",
                        yaxis=dict(autorange="reversed"),
                        yaxis_title="Magnitude",
...
def render_comp_validation(pipeline: Any, draft_id: int | None) -> None:
    ...
    st.caption("Validation of comparison star stability. Red = potentially variable.")
    ...
    if cs is None or cs.empty or "catalog_id" not in cs.columns:
        st.warning("Missing `platesolve/comparison_stars.csv`.")
        return
    comp_ids = [normalize_gaia_source_id(x) for x in cs["catalog_id"].tolist() if normalize_gaia_source_id(x)]
    ...
    if not rows:
        st.warning("No data for comparison stars in sidecars.")
        return
...
def render_variable_detection(pipeline: Any, draft_id: int | None) -> None:
    st.subheader("Variable Detection — Unknown variable stars")
    st.caption("Stars that are not in catalogs but show variability in the time series.")
    ...
    if ms is None or ms.empty:
        st.warning("Missing `platesolve/masterstars_full_match.csv`.")
        return
    if "catalog_id" not in ms.columns or "x" not in ms.columns or "y" not in ms.columns:
        st.warning("Masterstars CSV does not have expected columns.")
        return

    ms = ms.copy()
    ms["catalog_id_key"] = ms["catalog_id"].map(normalize_gaia_source_id)
    for col in ("vsx_known_variable", "catalog_known_variable", "gaia_dr3_variable_catalog"):
    ...
    if rms_df.empty:
        st.warning("Not enough frames to compute RMS.")
        return
    ...
    if not suspicious_detail:
        st.info("No suspicious stars at current thresholds.")
        st.markdown(
            "#### Info\nStars added to VAR tracking will be included in the PSF model "
            "on the next MASTERSTAR generation run (after updating the selection in the pipeline)."
        )
        return

    opts = [f"{d['name']} ({d['catalog_id_key']})" for d in suspicious_detail]
    pick = st.selectbox("Select star for detail", options=opts, key="vyvar_vdet_pick")
    ...
    if msel.empty:
        st.caption("Masterstars do not have this `catalog_id` — Gaia metadata is not available.")
        mrow = None
    else:
        mrow = msel.iloc[0]
    ...
    for d in suspicious_detail:
        ...
        with cols[1]:
            if st.button("Add to VAR tracking", key=f"vyvar_add_var_{cid}"):
                ...
                if vpath.is_file():
                    old = pd.read_csv(vpath, low_memory=False, dtype=_GAIA_ID_DTYPE)
                    existing = set(old.get("catalog_id", pd.Series(dtype=str)).map(normalize_gaia_source_id))
                    if cid not in existing:
                        ...
                        try:
                            ...
                            if "catalog_id" in old.columns:
                                old["catalog_id"] = normalize_gaia_source_id_series(old["catalog_id"])
                        except Exception:  # noqa: BLE001
                            pass
                    ...
                    try:
                        ...
                        if "catalog_id" in df_new.columns:
                            df_new["catalog_id"] = normalize_gaia_source_id_series(df_new["catalog_id"])
                    except Exception:  # noqa: BLE001
                        pass
                    df_new.to_csv(vpath, index=False)
                ...
                st.success(f"Added: {d['name']}")
        with cols[2]:
            if st.button("Mark as stable", key=f"vyvar_excl_{cid}"):
                ex = list(st.session_state.get("vyvar_var_detection_exclusions") or [])
                if cid not in ex:
                    ex.append(cid)
                st.session_state["vyvar_var_detection_exclusions"] = ex
                st.success("Marked — will disappear from the list after rerun.")

    st.markdown(
        "#### Info\nStars added to VAR tracking will be included in the PSF model "
        "on the next MASTERSTAR generation run."
    )
