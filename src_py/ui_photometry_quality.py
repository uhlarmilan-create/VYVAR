...
def render_photometry_quality_diagnostic(*, pipeline: Any, draft_id: int | None) -> None:
    ...
    st.caption("MASTERSTAR zones (linear / saturated / noisy), histograms, and per-frame overview. Charts are interactive (Plotly).")
    ...
    if (not archive_path) or (not os.path.isdir(str(archive_path))):
        st.info("Enter a valid draft path.")
        return
    ...
    if not ms_path.is_file():
        st.warning(f"Could not find `{ms_path}`. Run **MAKE MASTERSTAR** in the VAR-STREM tab.")
        return

    ms = _read_csv_cached(str(ms_path))
    if ms.empty:
        st.warning("`masterstars_full_match.csv` is empty.")
        return
    ...
    if not bg_fits_path.is_file():
        st.warning(
            f"Missing background FITS: `{bg_fits_path}`. "
            f"Leave the field empty for `{ms_fits_path.name}` or enter a valid path."
        )
        return

    for c in ("x", "y"):
        if c not in ms.columns:
            st.error(f"`masterstars_full_match.csv` is missing column `{c}`.")
            return
    ms_xy = ms.copy()
    ...
    st.markdown("## Per-frame CSV overview")
    if not per_frame_root.is_dir():
        st.info(f"Could not find `{per_frame_root}`.")
        return
    ...
    if not frame_metrics:
        st.info("No per-frame CSV found, or they are empty.")
        return
    fm = pd.DataFrame(frame_metrics)
    fm = fm.sort_values("jd_mid", ascending=True, na_position="last")
    st.dataframe(fm[["file", "jd_mid", "n_matched", "n_saturated", "n_usable", "status"]], width="stretch", height=320)
    ...
    if "peak_dao" not in ms_xy.columns or "peak_max_adu" not in ms_xy.columns:
        st.info("Missing `peak_dao` or `peak_max_adu` in MASTERSTAR CSV.")
        return
