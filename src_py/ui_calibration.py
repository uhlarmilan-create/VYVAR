...
def _preview_calibration_status(plan: Any, gks: list[str]) -> str:
    ...
    if getattr(plan, "quick_look", False):
        return "Quick look (no suitable master dark)"
    if flat_ok and dark_ok:
        return "Ready for calibration"
    if not flat_ok:
        return "Missing master flat (group)"
    return "Missing master dark (group)"
    ...
    *,
    ...
    for (f, e) in sorted(combos.keys(), key=lambda x: (x[0], x[1])):
        ...
        if cal_phase == "running":
            ...
            ps_st = "Done (MASTERSTAR)" if _plate_solve_ok_for_groups(ap, gks) else "Pending / not done"
        else:
            cal_st = _preview_calibration_status(plan, gks)
            ps_st = "After calibration and step 3"
