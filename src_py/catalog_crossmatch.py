...
class CrossmatchResult:
    ...
    def catalog_summary_bullets(self) -> list[str]:
        ...
        for cat in preferred:
            ...
            if err:
                lines.append(f"{cat}: error ({err})")
            elif not lst:
                lines.append(f"{cat}: no match")
            else:
                head = lst[0]
...
def _query_vsx_local_point(db_path: str | Path, coord: SkyCoord, radius_arcsec: float) -> list[CatalogMatch]:
    """Nearest VSX rows from local SQLite (``vsx_data``) within ``radius_arcsec`` (no network)."""
    ...
    r_deg = max(float(radius_arcsec) / 3600.0 * 1.5, 0.002)
    ra0 = float(coord.ra.deg)
    de0 = float(coord.dec.deg)
    cosd = max(0.2, abs(math.cos(math.radians(de0))))
    ra_min, ra_max = ra0 - r_deg / cosd, ra0 + r_deg / cosd
    dec_min, dec_max = de0 - r_deg, de0 + r_deg
    ...
    out.sort(key=lambda m: float(m.delta_r or 1e9))
    return out
...
def check_candidate_in_catalogs(
    ...
    *,
    ...
    vsx_p = str(vsx_local_db_path or "").strip()
    if vsx_p:
        try:
            loc_vsx = _query_vsx_local_point(vsx_p, coord, float(radius_arcsec))
            if loc_vsx:
                matches["VSX"] = loc_vsx
        except Exception as exc:  # noqa: BLE001
            errors["VSX (local)"] = str(exc)

    workers: list[tuple[str, Callable[[SkyCoord, float], list[CatalogMatch]]]] = [
        (lab, fn) for lab, fn in _CATALOG_WORKERS if not (lab == "VSX" and matches.get("VSX"))
    ]
    ...
    with ThreadPoolExecutor(max_workers=6) as ex:
        futs = {ex.submit(_job, lab, fn): lab for lab, fn in workers}
        try:
