...
def optimize_masterstar_matches(
    *,
    ...
    # Read IDs as strings to prevent float64 precision loss for 19-digit Gaia IDs.
    df = pd.read_csv(csv_path, low_memory=False, dtype={"catalog_id": str, "name": str})
    if df.empty:
    ...
    if id_col is None:
        ...
        cid = normalize_gaia_source_id_series(df.get(id_col, pd.Series([""] * len(df))))
    ...
    try:
        ...
        cid_s = normalize_gaia_source_id_series(df.get("catalog_id", pd.Series([""] * len(df), index=df.index)))
    ...
    n_all = int(len(df))
    n_match = int(
        normalize_gaia_source_id_series(df.get("catalog_id", pd.Series([""] * len(df)))).astype(str).str.strip().ne("").sum()
    )
