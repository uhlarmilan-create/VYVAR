...
def airmass_detrend_lc(
    ...
    *,
    ...
    coeffs = _safe_polyfit(am_fit, mag_fit, 1)
    if coeffs is None:
        return mag_calib.copy(), float("nan"), float("nan")
    slope, intercept = float(coeffs[0]), float(coeffs[1])

    if math.isfinite(slope) and abs(slope) > 0.5:
        _tid = str(target_id).strip() or "?"
        log_event(
            f"[AIRMASS] Varovanie: slope={slope:.3f} mag/airmass "
            f"(>0.5 — možné mraky alebo zlá kalibrácia). "
            f"Target: {_tid}"
        )
