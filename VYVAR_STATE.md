...
| File | Holds |
|------|-------|
| `VYVAR_ROADMAP.md` | Open work (the only place to look for "what's next"). |
| `VYVAR_DECISIONS.md` | Durable design decisions + *why* they hold. |
| `VYVAR_JOURNAL.md` | Chronological session log (history, append-only). |
| `VYVAR_PROCESS.md` | How we work: Definition of Done, byte-identity discipline, config↔UI parity, tests. |
| `VYVAR_PARAMS.md` | Config-key ↔ default ↔ clamp ↔ UI-location registry. |
