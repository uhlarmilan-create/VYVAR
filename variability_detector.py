...
def _lookup_cached_frame_df(p: Path, csv_cache: dict[str, pd.DataFrame] | None) -> pd.DataFrame | None:
    if not csv_cache:
        return None
    key = str(p)
...
def _process_variability_frame(
    p: Path,
    df: pd.DataFrame,
    *,
    flux_col: str,
    ...
    kept_cols: list[str],
) -> tuple[pd.DataFrame | None, str, float]:
    """Per-frame filters/dedupe (same rules as disk path). Returns (sub, frame_stem, bjd_median)."""
    if "catalog_id" not in df.columns or flux_col not in df.columns:
        return None, p.stem, float("nan")

    sub = df[[c for c in kept_cols if c in df.columns]].copy()
    sub["catalog_id"] = sub["catalog_id"].map(normalize_gaia_source_id)
    ...
    sub["_frame"] = p.stem
    bjd_median = float("nan")
    if "bjd_tdb_mid" in sub.columns:
        bv = pd.to_numeric(sub["bjd_tdb_mid"], errors="coerce")
        bjd_median = float(bv.median()) if bool(np.isfinite(bv).any()) else float("nan")
    return sub, p.stem, bjd_median


def _finalize_flux_matrix_from_rows(
    ...
    *,
    ...
    if not rows:
        raise FileNotFoundError(f"Failed to load proc frames (flux_col={flux_col})")
    ...
    bjd_array = np.asarray(
        [float(frame_bjd.get(str(c), float("nan"))) for c in pivot_norm.columns],
        dtype=float,
    )
    return pivot_norm, meta, bjd_array


def _flux_matrix_from_pivot(
    flux_pivot: pd.DataFrame,
    per_frame_dir: Path,
    ...
    *,
    ...
    """Build variability matrix from Phase 2A CSV cache (no disk read).

    ``flux_pivot`` documents the pre-built shape; processing uses ``csv_cache`` so filters
    match the disk path.
    """
    if not csv_cache:
        raise ValueError("csv_cache required for pivot fast path")
    ...
    if not frames:
        raise FileNotFoundError(f"No proc_*.csv in: {per_frame_dir}")

    kept_cols = list(_VARIABILITY_FRAME_COLS)
    if flux_col not in kept_cols:
        kept_cols.append(flux_col)

    rows: list[pd.DataFrame] = []
    frame_bjd: dict[str, float] = {}
    n_cache = 0
    for p in frames:
        df = _lookup_cached_frame_df(p, csv_cache)
        if df is None:
            continue
        n_cache += 1
        sub, stem, bjd_median = _process_variability_frame(
            p, df, flux_col=flux_col, zone_filter=zone_filter, kept_cols=kept_cols
        )
        if sub is None:
            continue
        rows.append(sub)
        frame_bjd[stem] = bjd_median

    LOGGER.info(
        "[VARIABILITY] Pivot fast path: %s frames from cache (pivot shape %s), 0 disk reads",
        n_cache,
        tuple(flux_pivot.shape) if flux_pivot is not None else (),
    )
    return _finalize_flux_matrix_from_rows(
        rows, frame_bjd, flux_col=flux_col, min_frames_frac=min_frames_frac
    )
...
def load_field_flux_matrix(
    ...
    *,
    ...
    cfg = config or {}
    logging.info(
        "[PERF-6] load_field_flux_matrix: csv_cache=%s, flux_pivot=%s",
        "ProcFrameStore"
        if csv_cache is not None and hasattr(csv_cache, "get_frame")
        else ("dict" if csv_cache else "None"),
        "provided" if flux_pivot is not None else "None",
    )
    try:
    ...
    if flux_pivot is not None and csv_cache:
        try:
            return _flux_matrix_from_pivot(
                flux_pivot,
                per_frame_dir,
                csv_cache,
                flux_col=flux_col,
            ...
            LOGGER.warning(
                "[VARIABILITY] Pivot fast path failed, falling back to disk: %s", exc
            )
    ...
    for p in frames:
        ...
        try:
            ...
            logging.warning("proc CSV process failed (%s): %s", p.name, exc)
            continue
        if sub is None:
            continue
        rows.append(sub)
    ...
    if n_cache and not n_disk:
        LOGGER.info(
            "[VARIABILITY] Loaded %d frames from Phase 2A cache (0 disk reads)", n_cache
        )
    elif n_cache:
        LOGGER.info(
            "[VARIABILITY] Loaded %d frames from cache, %d from disk", n_cache, n_disk
        )
...
def compute_rms_variability(
    ...
    *,
    ...
    try:
        comp_floor_factor = float(cfg.get("variability_comp_floor_factor", 1.5))
    except Exception:  # noqa: BLE001
    ...
    if vsx_targets_csv is not None:
        try:
            ...
            if vtp.exists():
                vtdf = read_vyvar_csv(vtp, low_memory=False)
                if "catalog_id" in vtdf.columns:
                    vtdf["_cid"] = vtdf["catalog_id"].map(normalize_gaia_source_id)
    ...
    out["vsx_known_variable"] = (
        out["vsx_known_variable"].fillna(False).astype(bool)
        | out["vsx_match"].fillna(False).astype(bool)
    )
    ...
    # Envelope for candidates (field hockey-stick fit)
    ...
    if comp_catalog_ids:
        for _cid in comp_catalog_ids:
            _nc = normalize_gaia_source_id(_cid)
            if _nc:
                _comp_id_set.add(_nc)
    if _comp_id_set:
        _cid_norm = out["catalog_id"].map(normalize_gaia_source_id)
        _comp_mask = _cid_norm.isin(_comp_id_set)
        _comp_sub = out.loc[_comp_mask].copy()
        _cmag = pd.to_numeric(_comp_sub.get("mag"), errors="coerce")
        _crms = pd.to_numeric(_comp_sub.get("rms_pct"), errors="coerce")
        _cok = _cmag.notna() & _crms.notna() & np.isfinite(_cmag) & np.isfinite(_crms) & (_crms > 0)
        if int(_cok.sum()) >= 3:
            _comp_fit = _comp_sub.loc[_cok].copy()
            _comp_fit["mag_bin"] = np.floor(pd.to_numeric(_comp_fit["mag"], errors="coerce")).astype("Int64")
            _comp_p90 = _comp_fit.groupby("mag_bin")["rms_pct"].quantile(0.90)
            LOGGER.info(
                "[VARIABILITY] Comp P90 rms_pct per mag bin: %s",
                {int(k): round(float(v), 2) for k, v in sorted(_comp_p90.items())},
            )
            _mag_all = pd.to_numeric(out["mag"], errors="coerce")
            out["_mag_bin"] = np.floor(_mag_all).astype("Int64")
            _exp_ser = pd.to_numeric(out["expected_rms_pct"], errors="coerce")
            _comp_noise_floor = out["_mag_bin"].map(_comp_p90).fillna(_exp_ser * 2.0)
            upper = np.maximum(upper, _comp_noise_floor.to_numpy(dtype=float) * float(comp_floor_factor))
            _p90_12 = float(_comp_p90.get(12, float("nan")))
            if not math.isfinite(_p90_12):
                try:
                    _p90_12 = float(_comp_p90.loc[12])
                except Exception:  # noqa: BLE001
                    pass
            _mask_mag12 = (out["_mag_bin"] == 12).fillna(False).to_numpy(dtype=bool)
        ...
        out.drop(columns=["_mag_bin"], errors="ignore", inplace=True)
    ...
    # 1) clipping ratio
    num = pd.to_numeric(out["n_frames_used_clean"], errors="coerce")
    den = pd.to_numeric(out["n_frames_used"], errors="coerce")
    ratio = np.where(den > 0, num / den, np.nan)
    out["clip_ratio"] = ratio
    ok_clip = pd.Series(ratio, index=out.index).ge(float(clip_ratio_min)).fillna(False)
    ...
    # Safe: mean_flux_norm_clean > 0.001 threshold excludes near-zero flux stars.
