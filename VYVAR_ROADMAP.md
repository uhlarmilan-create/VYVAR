...
- **Misc LOW:** TODO-7 plate-solver refactor · TODO-11 auto-trigger watchdog (`night_run`
  ...
  · TODO-PIXEL-XCHECK-BINNING binning-aware pixel cross-check (cosmetic log) · TODO-9 full
  VYVAR install on Lenovo T460 · TODO-PLATESCALE-PERSET focal×pixel per-set plate-scale
  fallback.

## FUTURE

- **Blind index — 3rd rig tier (Noctutec 206/560).** When a validated draft exists, add a
  manifest tier with measured `ρ_img` / plate scale; architecture is ready (`blind_index_series.json`
  + `blind_index_select_mode=auto`).
