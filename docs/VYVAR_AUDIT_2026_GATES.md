...
| G-QC-05 | INV-PREP-01 gradient ratio | `invariants_runtime.py:446` | WARN@10x | WARN |
...
| **Boundary (510)** | comp_rms max **0.025** - **75% below** 0.05 and **4x below** 0.10; **no comps within 20% of any ceiling** |
...
| **Condition** | SAT-DIAG: `n_over_admission/n_frames > 0.10` - `sat_diag.py:737`; photometry: peak > `admission_sat_peak_frac x sat_adu` (0.70) |
...
| **Per draft** | Per obs_group x dark x binning |
...
| **Condition** | Was `len(z) >= 4` ? 3xMAD clip |
...
| **Purpose** | Drop frames where alignment residual > frac x aperture radius |
