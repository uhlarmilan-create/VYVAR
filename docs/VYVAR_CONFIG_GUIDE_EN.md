...
| `global_comp_pool_enabled` | True | Setting (config.json) | config.json | config assembly & validation (`config.py`) | Builds one field-wide comparison pool reused across targets instead of fully per-target pools. |
