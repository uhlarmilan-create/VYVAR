...
| `frame_quality_fwhm_factor` | 1.0; range 0.8 .. 3 | Setting (config.json) | config.json | photometry engine (Phase 2A) (`photometry_core.py:7143`) | Scaling of the FWHM threshold used by the frame-quality gate. |
| `frame_quality_gate_enabled` | False | Setting (config.json) | config.json | Settings UI (`ui_settings.py:1101`) | Optional gate excluding frames of clearly worse sharpness than the night's typical seeing. |
| `frame_quality_min_keep_frames` | 10; range 3 .. 100000 | Setting (config.json) | config.json | photometry engine (Phase 2A) (`photometry_core.py:7148`) | Minimum number of frames that must survive the frame-quality gate. |
| `frame_quality_ratio_k` | 5.0; range 2 .. 20 | Setting (config.json) | config.json | photometry engine (Phase 2A) (`photometry_core.py:7142`) | How many times worse than the typical frame a frame must be before the quality gate drops it. |
...
| `variability_sigma_clip` | 5.0 | Setting (config.json) | config.json | config assembly & validation (`config.py:2411`) | Sigma-clipping level applied to light curves before variability metrics. |
...
| Parameter | Default | Type | Source | Used in | Explanation |
|---|---|---|---|---|---|
| `comp_clip_sigma` | 5.0; range 3 .. 10 | Setting (config.json) | config.json | Settings UI (`ui_settings.py:1132`) | Sigma-clipping level applied to comparison-star series before ensemble statistics. |
