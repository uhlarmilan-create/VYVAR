...
| 10 | G5-F011 split CT/AC consumers (export vs PDF) | **FIXED:** canonical `mag_calib_final` = `mag_calib` + CT + AC; export + all publication figures; G5-F003 precedence subsumed | (this commit) |
...
| G5-F011 | **FIXED** | L11 | `photometry_core.py`, `export_reports.py`, `photometry_report.py` | ~~Parallel CT/AC on `mag_calib`; export used AC, main PDF used CT — no combined column~~ **FIXED** (see fix log step 10): `mag_calib_final` = `mag_calib` + CT + AC; LC CSV + export + all publication LC figures; `lc_rms`/trust stay on `mag_calib`. | All publication-facing figures and export must use the same canonical calibrated magnitude. |
