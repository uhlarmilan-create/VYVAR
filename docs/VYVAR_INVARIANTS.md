...
| INV-COMP-MEMBERSHIP | Comparison-star membership in the ensemble is decided ONCE per draft by a selection rule, never per frame. A star is either a valid comparison for the observation or it is not. Membership that changes frame to frame converts a smooth systematic into a step function in the zeropoint and is forbidden regardless of the criterion used to decide it. | policy (code: no per-frame ZP rejection in `ensemble_normalize`) | FAIL (review) | Draft 509 BO CVn: per-frame 3xMAD ZP clip at N>=4 produced ~50 mmag two-state ZP; see DECISIONS ZP-CLIP-REMOVAL-2026-08-12. Separate from the general no-clipping iron rule. |
| INV-SAT-01 **[planned]** | Saturation and linearity limits are expressed in **image ADU** for the active `(equipment, readout mode, XBINNING, YBINNING)`; star peaks for limit comparison are measured on **raw** frames with a self-check (local maximum + ring contrast); `sat_limit_source` provenance is recorded. Aligned/resampled frames must not be the sole saturation authority when raw exists. Tier 3 (`DEFAULT_FRAC`) must not trigger exclusion. | spec only (`dev/results/specs/VYVAR_SAT_DIAG_SPEC.md`; not wired) | FAIL (planned) | SAT-DIAG spec 2026-08-13; survives config reduction unlike CAL-DIAG (`967f835`). See `dev/results/MEMO_saturation_limit_literature.md`. |
| INV-GATE-REMOVAL **[recommended]** | A verification gate may not be removed or disabled on byte-identity evidence alone. A passing gate produces identical outputs by construction, so byte-identity cannot distinguish a removed gate from a retained one. Removing a gate requires either a demonstration that the condition it checked is now impossible, or an explicit recorded decision to accept the unverified condition, with the risk stated. | policy (not wired) | - | CAL-DIAG removal `967f835`; same structural limitation as INV-ANCHOR-00. Architect recommendation 2026-08-13; see `dev/results/specs/VYVAR_SAT_DIAG_SPEC.md` section 13. |
