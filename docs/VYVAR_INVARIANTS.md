...
| INV-SAT-01 **[wired]** | Saturation and linearity limits are expressed in **image ADU** for the active `(equipment, readout mode, XBINNING, YBINNING)`; star peaks for limit comparison are measured on **raw** frames with a self-check (local maximum + ring contrast); `sat_limit_source` provenance is recorded. Aligned/resampled frames must not be the sole saturation authority when raw exists. Tier 3 (`DEFAULT_FRAC`) must not trigger exclusion. | both (`sat_diag.py` + `invariants_runtime.check_sat_diag`) | FAIL | SAT-DIAG spec 2026-08-13; Milan authorized 2026-08-13. See `dev/results/specs/VYVAR_SAT_DIAG_SPEC.md`. |
