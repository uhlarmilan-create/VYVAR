...
| INV-ANCHOR-00 **[audit finding]** | **Anchor `--full` gate coverage boundary.** `session_baseline_check.py --full` copies frozen inputs (`_copy_frozen_anchor_inputs`: platesolve tree minus photometry/_hrd_cache/*.pdf; aligned lights; `calibrated/lights/qc_metrics.csv` (APERTURE-01d night FWHM); cal_diag/draft_manifest/sat_diag) then empties dest photometry/ and reruns photometry only. Canonical freeze: `draft_000516_snapshot_era04_20260826` (core 9367f998 n=160). It does **not** exercise calibration, preprocess, alignment, stacking, MASTERSTAR build, DAO detection, or catalogue construction. | reference (not wired as FAIL) | - | CLOSE-OUT C7; APERTURE-01d; `CURSOR_RESULT_masterstar_count_diag.md`; Audit Stage 3 Part 0b. |
