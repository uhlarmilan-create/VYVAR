...
| INV-CFG-01 **[wired]** | Config?behavior no-op: when a gating flag is OFF, its provenance markers are ABSENT. Wired trio: `psf_photometry_enabled=False` ? no method=="psf" LC rows; `temporal_binning_enabled=False` ? no binning-applied markers; `cog_aperture_correction_enabled=False` ? no cog meta keys. | runtime (end-of-run) | FAIL | `validate_config_behavior`. |
