...
| INV-COMP-RMS-01 **[wired]** | Comparison candidacy RMS is `comp_rms_loo_mag` (1.4826 x MAD of leave-one-out differential mag vs the candidate pool, all loadable proc frames, no clipping). Ceiling is `min(phase01_comparison_max_comp_rms, k x photon)` with `photon = 1.0857 / snr_ap_pixscaled` and `k = comp_rms_loo_photon_k` (C3-0: 5). Missing `snr_ap_pixscaled` raises. Absolute cap 0.1 mag. `comp_relflux_mad` is diagnostic only. | test `dev/tests/test_c3_comp_rms_loo.py` + selector | FAIL (raise if SNR missing) | COMP-RMS-DEF-01-B; CLOSE-OUT C3. |
