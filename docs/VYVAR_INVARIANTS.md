...
| INV-PREP-01 **[wired]** | Post-preprocess large-scale gradient guard: ``large_small_ratio = var(blur(sigma=30)) / var(frame-blur)`` on one QC frame per obs_group; WARN above 10x (anchor ~1-5x; SKIPPROC regression 20-60x). | runtime (preprocess QC) | WARN | ``check_preprocess_large_small_ratio`` in ``_qc_enrich_calibrated_in_place``. |
