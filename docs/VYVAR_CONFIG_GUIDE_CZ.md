...
| `saturate_limit_fraction` | 0.80 | Nastaveni (config.json) | pouze vychozi v kodu | kalibrace a zpracovani snimku (`pipeline.py:6097`) | Podil saturacni urovne detektoru, nad kterym se hvezda povazuje za saturovanou a vylouci se z fotometrie. INV-SAT-LIMIT: 0.80 x 65535 = 52428 ADU, pokud neni zmereno linearity knee. |
...
| `epsf_auto_run` | False | Nastaveni (config.json) | config.json | orchetrace night-run (`night_run.py`) | Kdyz true, RUN VYVAR / night_run automaticky spusti ePSF stupen po aperturni fotometrii. Vychozi VYPNUTO. Odlisne od `psf_photometry_enabled` (sloupce psf_* Phase 2A). Tlacitka ePSF ve UI stupen vynuti. |
...
| `psf_ac_policy` | p4_none | Nastaveni (config.json) | config.json | F6 ePSF merge (`epsf_psf_merge.py`, `psf_photometry.py`) | Politika aperturni korekce F6. `p4_none` razitkuje neopravene fit flux (`psf_ac_factor=1`). `chi2_lt5_legacy` je pojmenovany fallback: median DAO/PSF mezi hvezdami s chi2<5 (vada EPSF-AC-01 A2, jasovy rez). |
| `psf_zp_membership` | fit_ok_for_zp | Nastaveni (config.json) | config.json | interni PSF LC (`psf_internal_lc.py`) | Clenstvi INV-PSF-LC-PIN-01 ZP. `fit_ok_strict` pouziva ulozene `psf_fit_ok`. `fit_ok_for_zp` pripusti i epochy s konecnym `psf_flux>0` a konecnym `psf_chi2`. Produkce: `fit_ok_for_zp` jen na overenych rigech. |
| `psf_zp_for_zp_validated_rigs` | ["1:1"] | Nastaveni (config.json) | config.json | interni PSF LC (`psf_internal_lc.py`) | Klic identity rige `equipment_id:telescope_id` povoleny pro `fit_ok_for_zp`. Draft 516/517 wide par je `1:1`. Ostatni rige zustavaji `fit_ok_strict` (EPSF-ZP-OK-XRIG-01). |
...
| `global_comp_pool_enabled` | True | Nastaveni (config.json) | config.json | sestaveni a validace konfigurace (`config.py`) | Stavi jeden spolecny fond srovnavacich hvezd pro cele pole misto plne oddelenych fondu po cilech. |
| `comp_pool_derived_admission` | True | Nastaveni (config.json) | config.json | globalni comp pool (`photometry_core.build_global_comp_pool`) | COMP-POOL-01: prijmani do fondu podle draftem odvozenych kriterii sumu/stability/diluce. |
| `comparison_stars_pool_n` | 0 | Nastaveni (config.json) | config.json | planovaci fond (`pipeline.select_comparison_stars_spatial_grid`) | Velikost planovaciho fondu; 0 = bez stropu (COMP-POOL-01). |
