...
| **DRAFT451-CAL-FRAME001** | MED | Draft 451 frame-001 calibrated product differs from draft 452 by **659.6 ADU** max while siblings differ ~121 ADU; raw 451 vs 452 inputs are byte-identical (0.0 ADU). Old-vs-new preprocess on the same night: **149/150** calibrated frames match exactly (452 vs 454); only frame 001 differs (**533.5 ADU**). Root cause of the 451 calibrated anomaly needs draft 451 calibration run logs / per-frame masters. Close when Milan supplies logs; otherwise stays open with missing evidence named. |
