...
| **EPSF-XVAL-01** | A2 measured R-A2-3 (target 10.48 / check 21.41 mmag, deg2, 134 identical epochs; PSFEx deg2 vs deg3 stable to ~0.1-0.3 mmag); closes on D-EPSF-XVAL-DOD-01 | Milan | D-EPSF-XVAL-DOD-01 |
...
- **EPSF-CORE-04** -- CLOSED 2026-09-15 measurement complete. Readings R-R2, R-P2, R-K0. PSFEx vs aperture 12.44 / 15.69 mmag (DoD reference re-decide is Milan's). osamp=4 sandbox ePSF pathological (non-finite phase surface); osamp=3 worse than osamp=2. No listed machinery knob lowers B1 vs aperture by >= 3 mmag. Architect error 25 recorded (R-Q4 ptp vs RMS). Evidence `CURSOR_RESULT_EPSF_CORE_04.md`.
