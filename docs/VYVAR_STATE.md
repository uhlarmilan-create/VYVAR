...
Architect errors 25-26 on the ledger. No code. Live 516 G4
prefixes unchanged (`bfa24039` / `13e77cf8` / `172f9540`).
**EPSF-CORE-04 2026-09-15.** Measurement complete:
...
EPSF-XVAL-01 stays OPEN (now D-EPSF-XVAL-DOD-02). No `psf_flux`
...
waited Milan GO (now C6-0 STOP). Live 516 ePSF SHA unchanged. Live 520 SHA unchanged.
...
not wired. PUSH_AUTH SHA `a23ee3d` on origin/sel-ghost-01.
...
X1 [WCS-APERTURE] refused: proc x,y identical (dx=0); centroid
residuals equal 0.797 px; D2 six cards did not move apertures.
X2 [CT-REF] `cfffa82`: BO `mag_calib_final` +2.787 mmag = `mag_calib`;
10/12 CT LCs collapse. X3 [EDGE] four `out_of_frame` (FR CVn, TIC
23815847, two Gaia VAR). mag_calib 2.8/10.8/6.8 UNNAMED (BO
aperture_r 5.999->5.499). No lock. Evidence:
`CURSOR_RESULT_ANCHOR_ERA04.md`.

## 2026-08-26 -- CLOSE-OUT C6-3 STOP (era04 not locked)

C6-0 R1'' informational shim then KeyError skip. C6-1 wrote era04
(core 961d590f n=169; 134 frames; live SHA unchanged). C6-2 `--full`
twice PASS same SHAs. C6-3: 55 UNNAMED; BO/FW/GH same ensemble but
mag_calib not 0/AC-constant. No lock. era03 freeze stays 9902d918 /
472bc9e4. Evidence: `CURSOR_RESULT_ANCHOR_ERA04.md`.
