...
| D10-1 | 4 | transforms | C?FIXED | - | unfiltered?CR band | closure #18 | FIXED Milan decision. **Annotate 2026-09-16 (DOD-04):** VAL-02 colour term c = +140 mmag/mag vs Johnson V on the PSF path (n=95 core) - input to the CV/CR decision. **Annotate 2026-09-16 (DOD-05):** CHROMATIC-PSF-01 records PSF-vs-aperture colour term c = +130 mmag/mag BP-RP on the same photons (VAL-03) | CLOSED |
...
| C-EXPORT-GAP | 3 | export | C | LOW | night_run omits AAVSO/VarAstro; UI/manual only | night_run.py vs export_reports.py | Document workflow | CLOSED-STALE measured 2026-09-07; CURSOR_RESULT_C_EXPORT_GAP_VERIFY_01.md |
...
| **D5-1** | 8 | aperture | U | MED | Q1: sizing FWHM and annulus geometry are per-draft frozen. Q2: per-star radius spread is `compute_snr_optimal_aperture_table`. **Annotate 2026-09-15 (DOD-03):** curve-of-growth correction is a prerequisite for any common-flux-scale use of the aperture path (VAL-01 criterion 2 signature -236..-548 mmag vs G; lit C4/C6). **Annotate 2026-09-16 (DOD-04):** VAL-03 delivers the measured growth-curve table (`cog_table.csv`) for the production CoG implementation. **Annotate 2026-09-16 (DOD-05):** VAL-03 `cog_table.csv` delivered; b_ap = +74.2 mmag/mag is the raw-aperture growth-curve signature. | `CURSOR_RESULT_COG_A1_01.md`; `docs/VYVAR_LITERATURE_CHECK_EPSF_20260915.md`; `CURSOR_RESULT_EPSF_VAL_03.md` | **ANSWERED** | CLOSED |
