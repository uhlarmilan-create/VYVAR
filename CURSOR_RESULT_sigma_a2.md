CURSOR RESULT — 2026-07-09 (SIGMA-A2)
...
### Joint fit (f_resid, sigma_floor) — draft_424 calibrator ensemble

| Parameter | Value | Bootstrap CI (16–84%) |
|-----------|-------|----------------------|
| f_resid | 0.74 | [0.0, 1.0] |
| sigma_floor | 10.5 mmag (0.0105 mag) | [9.5, 11.0] mmag |
| median chi2/dof | 1.000 | — |
| per-star IQR (floor variant) | 0.158 | — |

f_resid not pinned to edge. sigma_floor magnitude: 10.5 mmag (bookkeeping vs ~1–5 mmag PRNU/flat-residual scale: number only).

### Chi2/dof per calibrator — all four variants (draft_424, corrected rig)
...
5. G9.3 not excluded — saturation margin well below 0.85 pipeline fraction.
...
- `sigma_budget.py` — alt<=0 guard, `combine_sigma_mag_quadrature`, floor variant constant
- `scripts/chi2_sigma_gate.py` — joint fit + bootstrap CIs, saturation margin, 4th variant
- `scripts/select_constant_calibrators.py` — A2 rerun outputs, draft_425 trust probe
- `scripts/sparse_comp_diag.py` — chi2_ci/scatter_mag_ci naming, `--chi2-only`
