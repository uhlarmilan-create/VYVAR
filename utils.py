...
def parse_fits_binning_int(raw: object, default: int = 1) -> int:
    ...
    if isinstance(raw, (float, np.floating)):
        if math.isfinite(float(raw)) and float(raw) > 0:
...
def infer_binning_xy_from_sensor_shape(
    naxis1: int,
    naxis2: int,
    ...
    if hx > 1 or hy > 1:
        return hx, hy, False
    if native_size is None or naxis1 <= 0 or naxis2 <= 0:
        return hx, hy, False
    nw, nh = int(native_size[0]), int(native_size[1])
    if nw <= 0 or nh <= 0:
        return hx, hy, False
    tol = max(64, int(0.02 * max(nw, nh)))
    for k in (4, 3, 2):
        ew, eh = nw // k, nh // k
        if abs(int(naxis1) - ew) <= tol and abs(int(naxis2) - eh) <= tol:
            return k, k, True
    return hx, hy, False
...
def fits_binning_xy_from_header(header: fits.Header) -> tuple[int, int]:
    ...
    raw_x = header.get("XBINNING", header.get("BINNING", 1))
    xb = parse_fits_binning_int(raw_x, 1)
    yb = parse_fits_binning_int(header.get("YBINNING", xb), xb)
    return xb, yb
...
def dao_detection_fwhm_pixels(header: fits.Header | None, *, configured_fallback: float | None) -> float | None:
    ...
    - Else if ``VY_FWHM_GAUSS`` exists: same (2D Gaussian fit from MASTERSTAR).
    ...
    # Safe: header key checked with `in header` before access (VY_FWHM / VY_FWHM_GAUSS).
    if header is not None and "VY_FWHM" in header:
        try:
            ...
            pass
    if measured is None and header is not None and "VY_FWHM_GAUSS" in header:
        try:
            v = float(header["VY_FWHM_GAUSS"])
            if math.isfinite(v) and 0.5 < v < 80.0:
                measured = v
        except (TypeError, ValueError):
            pass
...
def maybe_rescale_linear_wcs_cd_to_target_arcsec_per_pixel(
    ...
    *,
    ...
    """Scale linear WCS (CD, PC+CDELT, or CDELT-only) so det(pixel_scale_matrix) matches ``target``.

    Only for **linear** WCS (no SIP on ``wcs_in``). Position angle unchanged; CRVAL/CRPIX untouched.
    """
    ...
    if not math.isfinite(current_scale) or current_scale <= 0:
        return wcs_in, False
    rel = abs(current_scale / tgt - 1.0)
    if rel <= trigger_relative_mismatch:
        return wcs_in, False
    fac = tgt / current_scale
    if abs(fac - 1.0) < 1e-4:
        return wcs_in, False
    ...
    try:
        if w.wcs.has_cd():
            w.wcs.cd = np.asarray(w.wcs.cd, dtype=np.float64) * fac
        elif w.wcs.has_pc():
            w.wcs.pc = np.asarray(w.wcs.pc, dtype=np.float64) * fac
            cdelt = np.asarray(w.wcs.cdelt, dtype=np.float64)
            if cdelt.size and not np.allclose(cdelt, 1.0):
                w.wcs.cdelt = cdelt * fac
        else:
    ...
    if abs(new_scale - tgt) / tgt > 0.05:
        logging.warning(
            "WCS rescale verification failed: target=%.4f arcsec/px got %.4f (fac=%.6f)",
            tgt,
            new_scale,
            fac,
        )
...
def resolve_draft_dir(
    draft_dir_override: str | Path | None = None,
    draft_id: int | None = None,
    archive_root: str | Path | None = None,
    *,
    drafts_before_session: bool = False,
) -> str | None:
    """
    Canonical draft directory resolution for VYVAR UI modules.

    Priority chain (default, ``drafts_before_session=False``):
    1. explicit ``draft_dir_override`` (only if existing directory)
    2. ``vyvar_last_job_output["archive_path"]`` from session state
    3. ``vyvar_post_cal_archive_path`` from session state
    ...
    if draft_dir_override is not None:
        p = Path(draft_dir_override).expanduser()
        if p.is_dir():
            return str(p.resolve())

    def _drafts_path() -> str | None:
        if draft_id is None or not archive_root:
            return None
        try:
            return str((Path(archive_root) / "Drafts" / f"draft_{int(draft_id):06d}").resolve())
        except (TypeError, ValueError):
            return None

    def _session_archive_paths() -> str | None:
        last_job = st.session_state.get("vyvar_last_job_output") or {}
        if isinstance(last_job, dict):
            jap = last_job.get("archive_path")
            if jap:
                s = str(jap).strip()
                if s:
                    return s
        post_cal = st.session_state.get("vyvar_post_cal_archive_path")
        if post_cal:
            s = str(post_cal).strip()
            if s:
                return s
        return None

    if drafts_before_session:
        p = _drafts_path()
        if p:
            return p
        return _session_archive_paths()

    s = _session_archive_paths()
    if s:
        return s
    return _drafts_path()


def resolve_draft_dir_path(
    draft_dir_override: str | Path | None = None,
    draft_id: int | None = None,
    archive_root: str | Path | None = None,
    *,
    ...
    """``resolve_draft_dir`` returning a ``Path`` (or ``None``)."""
    s = resolve_draft_dir(
        draft_dir_override,
        draft_id,
        archive_root,
        drafts_before_session=drafts_before_session,
    )
    if not s:
        return None
