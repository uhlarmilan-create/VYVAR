...
def step_2_load_targets() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    comp_df = pd.read_csv(comp_path, low_memory=False, dtype=_GAIA_ID_DTYPE)
    ...
    if PSF_TARGET_OVERRIDE:
        ...
        var_df = pd.read_csv(var_path, low_memory=False, dtype=_GAIA_ID_DTYPE)

    if "catalog_id" in var_df.columns:
        var_df = var_df.copy()
        var_df["catalog_id"] = var_df["catalog_id"].map(normalize_gaia_source_id)
    ...
    if "catalog_id" in comp_df.columns:
        ...
        comp_df["catalog_id"] = comp_df["catalog_id"].map(normalize_gaia_source_id)
...
def step_3_run_psf_on_frames(
    ...
    *,
    ...
    var_part = pd.DataFrame(
        {
            "catalog_id": var_df.get("catalog_id", pd.Series([], dtype=str)).map(normalize_gaia_source_id),
    ...
    comp_part = pd.DataFrame(
        {
            "catalog_id": comp_df.get("catalog_id", pd.Series([], dtype=str)).map(normalize_gaia_source_id),
            "name": comp_df.get("catalog_id", pd.Series([], dtype=str)).map(normalize_gaia_source_id),
    ...
    for i, csv_path in enumerate(frames, start=1):
        ...
        try:
            ...
            proc_df = pd.read_csv(csv_path, low_memory=False, dtype=_GAIA_ID_DTYPE)
            ...
            try:
                ...
                if not var_rows.empty:
                    ...
                    for _, vr in var_rows.iterrows():
                        ...
                        cid = normalize_gaia_source_id(vr.get("catalog_id"))
...
def step_4_build_summary() -> None:
    ...
    for fp in files:
        try:
            frames.append(pd.read_csv(fp, low_memory=False, dtype=_GAIA_ID_DTYPE))
        except Exception:  # noqa: BLE001
            continue
    if not frames:
...
def step_5_calibrate_lightcurve() -> None:
    ...
    summary["catalog_id"] = summary["catalog_id"].map(normalize_gaia_source_id)
    ...
    comp_ids = [normalize_gaia_source_id(x) for x in COMP_CALIB_CATALOG_IDS]
    ...
    try:
        for d in PSF_TARGET_OVERRIDE:
            cid0 = normalize_gaia_source_id(d.get("catalog_id"))
            if cid0:
