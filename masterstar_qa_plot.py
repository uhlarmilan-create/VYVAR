...
def build_starfield_qa_png_mapping(
    ...
    *,
    ...
    if mapped_df is not None and not mapped_df.empty and "x" in mapped_df.columns and "y" in mapped_df.columns:
        ...
        use_source_type = "source_type" in dfx.columns
        st_col = (
            dfx["source_type"].fillna("").astype(str).str.strip().str.upper().values
            if use_source_type
            else None
        )
        r = float(mark_r)
        ...
        if show_labels and "name" in dfx.columns:
            ...
            for i in range(len(dfx)):
                ...
                if use_source_type and st_col is not None:
                    stv = str(st_col[i])
                    if stv == "FORCED_APERTURE":
                        fill = (0, 200, 220) if not invert else (120, 240, 255)
                    elif stv == "GAIA_MATCHED":
                        fill = (240, 240, 0) if not invert else (200, 255, 120)
                    else:
                        fill = (255, 170, 170)
                else:
