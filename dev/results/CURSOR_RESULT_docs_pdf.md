CURSOR RESULT ù 2026-07-18 (DOCS-PDF)
...
## STEP 1 ù docs-layout guard relaxed (deliberately)
...
builder) ù kept verbatim per the commit-verbatim instruction; it is cosmetic and does not
affect page/entry counts.

### reportlab in requirements ù DEVIATION from STEP 2.4 (stated, on purpose)
...
RUNTIME dependency of the pipeline ù it renders the SUMMARY MEASURE REPORT PDFs:
...
## STEP 3 ù cross-links added

- `README.md` Documentation table: two rows ù Parameter handbook (Czech, PDF) and
...
only, NOT the PDFs ù PDF regeneration belongs to the docs-revision ritual.
...
## Push
HELD per STEP 4 ù awaiting Milan's PUSH protocol.
