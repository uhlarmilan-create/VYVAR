...
| **T1 direction stats** | Isotropic (vector mean 4.6″ ≪ mean\|Δ\| 12.3″); weak radius corr 0.246 — not rigid shift / not SIP-radius signature |
| **T3 LC positions** | **No per-frame x/y** in LC CSVs; `alignment_report.csv` frame-level only |
| **T4 81″ population** | **SPURIOUS-UNIFORM** — control ratio det/control = 1.07; diagnostic not biased |
...
v4 flagged **164 MISASSIGNED-ID** via `sep(WCS(x,y), Gaia[catalog_id]) > 3.9″`. v5 peak test shows
...
sky coords match Gaia within ~0.24″** (median).

| Quantity | Priority median | Interpretation |
|----------|----------------:|----------------|
| `sep_proj_px` (ms x/y ↔ Gaia WCS→pixel) | **1.3 px** | Same physical star in pixel space |
| `sep_wcs_gaia_arcsec` (pix2world(x,y) ↔ Gaia) | **12.8″** | v4 violation metric |
| `sep_vt_gaia_arcsec` (vt coords ↔ Gaia) | **0.24″** | catalog_id assignment correct |
...
## T1 — Direction statistics (Δ = WCS(x,y) − Gaia[cid])

| Set | n | vector mean | mean \|Δ\| | corr(\|Δ\|, r_field) |
|-----|--:|------------:|-----------:|---------------------:|
| MISASSIGNED (v4 class) | 160* | 4.6″ | 12.3″ | +0.246 |
| CONSISTENT (violation subset) | 20 | 1.0″ | 2.9″ | −0.248 |
...
Per-row loose catalog match at detection time (96″ class) is the plausible assignment path.
...
|--------|----------|------------:|-------------:|------------:|-----------:|
| FY CVn | CATALOG_PROJECTION_OFF | 1.21 | 11.8″ | 0.24″ | 0.76 |
| FZ CVn | CATALOG_PROJECTION_OFF | 1.31 | 12.8″ | 0.27″ | 0.39 |
| R CVn | CATALOG_PROJECTION_OFF | 1.42 | 13.9″ | 0.13″ | 3.54 |
| CSS_J134925.3+393524 | CATALOG_PROJECTION_OFF | 1.86 | 18.2″ | 0.21″ | 5.42 |
| NSVS 5096293 | CATALOG_PROJECTION_OFF | 0.66 | 6.5″ | 1.35″ | 18.78 |
| CSS_J140918.7+423422 | CATALOG_PROJECTION_OFF | 1.43 | 13.9″ | 0.99″ | 21.37 |
| RX CVn | CATALOG_PROJECTION_OFF | 0.86 | 8.4″ | 0.18″ | 0.08 |
...
## T4 — 81″ unmatched DET population
