...
check-star ?^2_red on anchor snapshot `draft_000435_snapshot_skysurface_20260716`, and
...
## 1b.3 - Production-path check-star ?^2_red

| Metric | Value |
|--------|------:|
| Check-star fields | 162 |
| Median ?^2_red (production path) | **649.0** |
| Median ?^2_red (target LC, same targets) | **187.6** |
...
| Method | Median ?^2_red | Valid constant-star test? |
|--------|--------------:|---------------------------|
...
1. **40.98** - Used `check_kmag_*` sidecar magnitudes with **reconstructed** uncertainty
   ...
   too-small err inflates ?^2 (consistent with ~95% SEM fraction in decomposition).

2. **0.108** - **Not** ?^2_red of the target light curve. Part 1 computed
   ?^2(`kmag_check`, `err_target` per frame) - check-star kmag divided by **target** errors.
   That pairs two different objects and artificially yields ?^2 ? 1 when target err is sized
   for variable targets, not check-star scatter. It is **not comparable** to 40.98 or to
   production check-star ?^2.

3. **649 (production path)** - Correct pairing: `mag_calib_final` and `err` from the same
   `_phase2a_process_one_target` run with the parent target's comparison ensemble. ?^2 ? 1
   ...
   0.058 mag but min 0.006 mag ? ?^2_red=110).
