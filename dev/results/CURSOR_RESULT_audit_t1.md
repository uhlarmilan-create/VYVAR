...
Photometry regen vs `draft_000435_snapshot_skysurface_20260716`: see `tmp/audit_t1_photometry_result.json` (run in progress at session close).
...
Test: `dev/tests/test_export_time_base_refusal.py` (JD_FALLBACK / absent / mixed ? no file, no `#DATE=BJD`).
