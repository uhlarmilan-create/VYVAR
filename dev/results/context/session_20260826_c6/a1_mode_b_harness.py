# -*- coding: ascii -*-
"""Mode (b) harness: f_per_frame differential mag on candidate1 aligned FITS.

Does not write era04. Used only for P-A2/P-A4 comparison vs mode (a) production.
"""
...
def main() -> int:
    ...
    for fp in lights:
        ...
        fw = resolve_frame_fwhm_px(
            hdr=hdr, frame_name=fp.name, qc_fwhm_by_name=qc_map, fwhm_night_median_px=night
        )
        if fw is None:
            fw = float(night) if night is not None else 5.19
        r_ap, r_in, r_out = resolve_aperture_geometry(
            f=F, fwhm_px=float(fw), annulus_inner_fwhm=ANN_IN, annulus_outer_fwhm=ANN_OUT
        )
    ...
    out = {
        ...
        "f": F,
        "n_frames": len(lights),
        "r_ap_min": round(min(r_aps), 4),
        "r_ap_max": round(max(r_aps), 4),
        "r_ap_median": round(float(np.median(r_aps)), 4),
        "stars": stars,
        "elapsed_s": round(time.perf_counter() - t0, 2),
    }
    (OUT / "a1_mode_b_harness.json").write_text(json.dumps(out, indent=2), encoding="ascii")
