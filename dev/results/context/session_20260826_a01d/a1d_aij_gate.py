# -*- coding: ascii -*-
"""APERTURE-01d independent gate: BO at f=1.35, annulus 2.7/5.2 (AIJ 14/27).

Same ensemble as APERTURE-01c (Table.tbl T1+C2..C6). Gate <= 2.8 mmag
(annulus now matched; expect improvement vs 2.7833).
...
SETUP = "NoFilter_60_2"
OUT = ROOT / "dev" / "results" / "context" / "session_20260826_a01d"
...
def main() -> int:
    ...
    rec: dict = {
        "task": "APERTURE-01d independent AIJ gate",
        "f": F,
        "annulus": [ANN_IN, ANN_OUT],
        "mode": "f_fixed_night",
        "aij_csv": str(AIJ_CSV),
        "aij_tbl": str(AIJ_TBL),
    ...
    if not ens.get("ok"):
        ...
        (OUT / "a1d_aij_gate.json").write_text(json.dumps(rec, indent=2), encoding="ascii")
    ...
    rec["aij_sky_in"] = ens.get("sky_in")
    rec["aij_sky_out"] = ens.get("sky_out")
    rec["qc_n"] = len(qc_map)
    ...
    rec["gate_mmag"] = GATE_MMAG
    rec["pass"] = bool(math.isfinite(rms) and rms <= GATE_MMAG)
    rec["elapsed_s"] = round(time.perf_counter() - t0, 2)
    (OUT / "a1d_aij_gate.json").write_text(json.dumps(rec, indent=2, default=str), encoding="ascii")
    print(
        "RMS=%.4f mmag n=%d r_ap=%.3f r_in=%.3f r_out=%.3f AIJ_r=%s gate=%.1f pass=%s elapsed=%.1fs"
        % (rms, rec["n_join"], r_ap, r_in, r_out, rec["aij_r_px"], GATE_MMAG, rec["pass"], rec["elapsed_s"])
    )
