...
1. **Reporting columns.** AAVSO: `mag_calib`/`mag_calib_ac`. PDF LC: `mag_calib_ct`/`mag_calib`.
   V0612 post-finalize: corr **~0.37** on shipped `mag_calib` (historical target-fit airmass detrend, now removed).
...
1. **Binning OFF default** � `config.py`, `config.json`, `ui_settings.py`, `dev/results/config_schema.md` (archived).
...
| Item | Evidence |
|------|----------|
| Target-fit airmass | **[Removed 2026-06-19]** Former per-target LSQ on the target curve (not comp-derived k). Was wired via `_apply_airmass_detrend_helper`; helper + `airmass_detrend_lc`/`_piecewise` deleted. Historical: DoD-A corr(`delta_mag`, `mag_calib`)=0.59 vs corr(`delta_mag`, `mag_calib_raw`)=0.998. |
...
| Where shape breaks | DoD-A LC: `corr(delta_mag, mag_calib_raw)=0.998`; historical post-detrend corr(`delta_mag`, `mag_calib`)=0.59 (slope ~0.78 mag/am) � detrend path since removed. |
