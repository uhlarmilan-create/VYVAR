...
Tranche 3 inferred ~9? effective significance from `conv/?_pixel ? 0.39�0.45` using a **unit-sum Gaussian smooth**. DAOStarFinder uses Stetson (1987) FIND's **zero-sum density-enhancement kernel** with `scale_threshold=True` (photutils default), so `threshold_eff = threshold � kernel.rel_err` and nominal `N�?_pixel` is **exactly N? in the convolved image** for white noise.
...
| measured ?_conv/?_pixel | 1.3594 |
| significance of 3.8�?_pixel threshold | **3.80 ?_conv** |

**Withdraw:** S3 recommendation to switch threshold source to Background2D *because ?_pp is unstable vs bkg2d*. M2 showed both estimators agree within ~1.2% per frame; 14�16% frame-to-frame spread is a property of resampling/dither, not estimator choice.
...
**Neither ?_pp nor Background2D fixes this** � both correctly estimate per-pixel ? of the resampled image; swapping estimators does not restore the uncorrelated-noise assumption.
...
Photometry path already handles correlation via Labb� empty apertures (empirical ? on resampled image). Detection should **measure noise of the quantity thresholded**, not derive from white-noise ?_pixel.

---

## �3 � M1/M2 discrepancy **resolved** (not a measurement bug)

Tranche 4 �6 flagged: M1 post-align ?_pp median 30.64 ADU vs M2 range 44.93�51.94 ADU for "subset" frames.
...
Post-align ?_pp on shared frame 001: **51.94 ADU in both** (exact match).

M1 median 30.64 is dominated by frames 017�150 (?_pp ? 28�32 ADU). M2 frames 002�005 were **never in M1**; they sit at ?_pp ? 45�51 ADU (less warp attenuation / different dither). The headline "align drops ?_pp ~28%" compares **different frame indices** (early vs late night), confounded with resampling � **must not be used** until a paired before/after study on the **same** frames is run.

**Action for future measurement:** paired cal/pre/aligned ?_pp on identical frame list; separate early vs late frame cohort if dither evolution is of interest.
...
| Item | Status |
|------|--------|
| Push AUDIT-T3 bundle (P-10 + ?_pp + 3.8) | Still strictly better than `origin/main`; not blocked |
