...
|---------|------------|--------|
| p50 ~81 arcsec looked 'impossible' | v1 used 0.05 deg SQL box ORDER BY without cos(dec); brute-force v2 **confirms** p50=81.053 arcsec is real for unmatched DET_* (not a query bug) | Field-bounded Gaia load (13427 rows) + vectorized cos(dec) NN |
| Self-match 26 arcsec with same catalog_id | v1 reported **nearest sky DAO** not **same-cid ms row** | Self-match joins vt.cid -> ms.cid; violations are vt VSX coords vs ms WCS coords (5-60 arcsec systematic) |
...
| p50 | **81.053 arcsec** |
| p90 | 132.006 arcsec |
| within 1x FWHM (6.234 arcsec) | 0/2724 |
| within 2x FWHM | 5/2724 |

Matched masterstars vs Gaia DB by source_id: p50 **2.05 arcsec** (sanity: astrometry OK for matched rows).

### DIAG SELF-CHECK: **FAIL** (expected - valid decision input)
1. median 81.053 arcsec >= 15 arcsec gate - unmatched DET detections are ~80 arcsec from nearest catalog source on median (why they remain unmatched).
2. 179/249 vt<->ms self-match violations > 2 arcsec - VSX export coords vs platesolved masterstar coords diverge (not a join bug).
