...
|-----|---------------|----------|
| **FIX 1** | `wcs_invertibility.py`; `pipeline.py` post-solve + post-optimizer | 9×9 round-trip gate; p99 < 0.2 px PASS; initial solve WARN+flag; optimizer refit FAIL-CLOSED |
...
| **FIX 4** | `astrometry_optimizer._write_match`; `detect_stars` post-match gate | WARN >1.5×FWHM px; drop assignment >3×FWHM px |
| **FIX 5** | `diag_428_coord_forensics_v5.py` | Poisson 33.4″ vs control 122.4″ reconciled (non-uniform cone export) |
...
apertures used the healthy Gaia branch → `world2pix(Gaia)` ≈ ms x/y but `pix2world(x,y)` ~12″ off.
...
5. masterstars: violations vs Gaia <2″ for matched rows; `coord_source` populated
