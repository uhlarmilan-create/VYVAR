...
- **T3:** Forensics **rejected** stale-WCS pass condition for unmatched DET_* (recomputed p50 81.141 arcsec ~ stored 81.053 arcsec). **No production coord fix** per task gate - Milan decision on optional matched-row Gaia coord refresh.
...
|--------|---------------|------------------------------------------|
| vector-mean magnitude | 0.114 arcsec | 0.226 arcsec |
| mean \|Delta\| to nearest Gaia | 84.771 arcsec | 84.777 arcsec |
| **p50 \|Delta\|** | **81.053 arcsec** | **81.141 arcsec** |
| corr(\|Delta\|, field-center dist) | 0.0034 | - |

**DIAG SELF-CHECK FAIL (forensics):** recomputed p50 >= 20 arcsec -> **stale-WCS hypothesis NOT confirmed** for unmatched DET_*. Large p50 reflects unmatched DAO detections without nearby Gaia (not a fixable global WCS offset).

**vt<->ms violation source (T1.3):** 179 violations analyzed -> **ms_deviates=176**, vt_deviates=3, tie=0. vt positions align with Gaia DB (VSX catalog coords); ms ra/dec retain detection-time WCS (5-60 arcsec off Gaia for matched IDs).
...
- [ ] diag v3 on fresh masterstars: unmatched DET p50 still ~81 arcsec (expected - not a coord bug); vt<->ms violations unchanged until optional COORD follow-up
- [ ] Radius decision input: re-run `--forensics` for within 1x/1.5x/2x FWHM counts; Milan decides radius - **no radius change in this arc**
