...
**Implementation:** `threshold = N x rms_conv`, `scale_threshold=False`, where `rms_conv` is robust RMS of the FIND kernel convolved detection image (`scipy.ndimage.convolve`, zero-sum kernel).
...
| Resampled frame: `rms_conv ? ?_pixel x rel_err` | **PASS** |
...
| Check-star ?^2 | - | deferred to post-N photometry re-run |
