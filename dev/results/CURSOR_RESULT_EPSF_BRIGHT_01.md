...
2. **Sky-only miscalibration LIKELY PRIMARY** — dim stars show chi2 ~ 1–3 (healthy); bright-end
   ...
   proxy `1 + F/(n_eff·?_sky²)` (n_eff=9) stays ~1.0 for all stars and **underpredicts** absolute
...
3. **Real shape misfit may add on top** for the very brightest (chi2 10³–10?) — cannot fully
...
**Cutout residual probe:** 30 samples on 5 brightest comp stars × 6 frames. Sandbox rebuild of
...
| Option | When | Action |
|--------|------|--------|
| **FD-A (preferred)** | M2 confirms miscalibration | Add source Poisson term to fit error map: ?² = F_model/g + sky/g + (RN/g)²; keep threshold 50 as genuine outlier gate; PSF columns only |
