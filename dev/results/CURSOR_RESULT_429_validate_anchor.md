...
| A6 | masterstars coords + v5 forensics | **PASS** | `coord_source`: gaia_catalog=2875, final_wcs=179. Matched sep vs Gaia DB: median 0.0 arcsec, p95 ~8e-11 arcsec, 0 rows >2 arcsec. v5 on 429: **`MISASSIGNED_164 n=0`** (`tmp/f429_coord_forensics_v5.txt`) - zero CATALOG_PROJECTION_OFF in the 164-cohort; priority T2 offsets ~1-7 arcsec sep_wcs_gaia (residual peak-test on 6 stars, not census misassignment). |
...
**1172 disappeared rows:** median mag **15.14**; sep(ms sky -> Gaia) median **17.0 arcsec**, p95 **80.5 arcsec**, **1142/1172** >2 arcsec - faint/noise end, loose astrometry consistent with SIP-displaced pass-2 boxes on 428.

**Verdict: CONFIRMED.** 428's inflated census (8927 raw, 2724 unmatched, +1172 spurious matched) is dominated by pass-2 DAO on Gaia targets searched through corrupt forward SIP (~12 arcsec bookkeeping offset per v5); 429 pass-2 (+1180) on healthy WCS yields 179 unmatched. Per-target science for well-detected stars unaffected (v5). **429 is the first healthy post-cleanup draft.**
