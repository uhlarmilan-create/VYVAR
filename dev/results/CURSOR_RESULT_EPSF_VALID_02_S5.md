...
HEAD: `2ba3d58`. Gate: no code change in this task; `--fast` not re-run (unchanged from S1-S4 PASS).

---

## Part B - written verdicts

### B1 - Contamination mechanism (production 1475-star model)
...
**Violation census - 1408 production-only stars (1475 ? 67 gated overlap):**
...
isolation but were never science-scoped. The production build (pre-Part C, MS-SOURCES-RETIRE pool)
...
### B2 - Cutout background vs I-11

**Verdict: correct (with documented double-subtraction caveat on already sky-subtracted frames)**

**What is subtracted:** In `_epsf_prepare_stars`, each 17-17 cutout uses a **2-pixel border median**
(`border[2:-2,2:-2]` excluded); median subtracted before `EPSFBuilder` (`psf_photometry.py` ~1283-1323).
Comment cites Anderson & King (2000) practice: local sky must be removed before PSF stacking because
**photutils `EPSFBuilder` does not subtract sky** (confirmed photutils 3.0.0 - no sky step in
...
proper unit-normalized PSF (documented in method Notes, lines 1360-1394). VYVAR meta records
...
-evaluate and subtract sky from each PSF star cutout- when the frame is already near zero mean - the
...
`EPSFBuilder` normalization forces unit flux sum - consistent with AK00 zero-sky PSF stars.

---

### B3 - Pixel weighting / variance model in fit

**Verdict: needs-measurement for CCD-equivalent full variance; current model is deliberate sky-only**

**EPSFBuilder (build):** `NDData(data)` only - **no uncertainty** passed to `extract_stars`
(`psf_photometry.py` ~1268-1298). photutils 3.0.0 `epsf_stars.py`: when uncertainty absent,
`EPSFStar.weights = ones_like(data)` - **uniform weighting** in ePSF construction iterations.

**EPSFFitter / PSFPhotometry (science photometry):** VYVAR passes `error=err_cut` from
`_psf_fit_error_cutout`: uniform map with ? = `sqrt(sky/g + (RN/g)-)` using annulus/border **sky
...
(`_psf_sandwich_flux_err`) with the same sky-only ? - **not** source Poisson.
...
frame values into the ePSF build; fit weights use explicit sky estimate, not raw pixel values - **avoids
the classic pedestal corruption**. Trade-off: weights ignore source Poisson term at the fit stage
(homogeneous ? map); chi2 is therefore -sky-only weighted- not full CCD-likelihood.
...
### B4 - Aperture-correction chain vs production DAO
...
1. `psf_photometry_stars` ? raw `psf_flux` (unit-normalized ePSF - fit flux).
...
## Part D - minimum certificate (gated 67-star pool, sandbox)
...
### D2 - Convergence (N = 15, 30, 50, 67 vs 67-star reference)

**Verdict: CONCERN - curve does not flatten until full gated pool**
...
**Break point:** n=15 included by design - largest deviation (~325 mmag). n=30 **worse** than n=15 in
...
**Proposed N policy (for meta/docs - architect + Milan confirm at STOP-B):**
...
| `--fast` | Not required (no code change); last PASS at S1-S4 HEAD `2ba3d58` |
| HEAD | `2ba3d58` |
...
- Odd-indexed 34-star ePSF build fails (non-finite ePSF after iterations) - D1 partial.
...
S5 completes written verdicts (B1-B4) and minimum certificate (D1/D2). **Do not proceed to S6**
...
3. D1 CONCERN - require odd-half build fix (centroid/edge stars) before swap, or accept even-vs-full stability?
