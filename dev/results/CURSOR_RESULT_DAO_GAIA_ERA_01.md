...
## A-fix 4 - Diagnostic measurement mode (DONE)
...
|------------|--------|---------------------------|
| detection_identity | Fresh pass1 DAO + greedy assign at R_diag; pass2 **accepted** at centroid cap 3.0 px | R_diag = max(10, 2*FWHM) = **10.39 px** |
| seed_centroid | Unowned G<=15: pass2 accepted + forced-seed SNR>=min (centroid gate not applied) | centroid measure cap **3.0 px** |

Tail handling: `random_match_scale = sqrt(1/(pi*R_diag^2))`; core = separations
...
Hand-validated reference: match **3.0 px**, centroid **2.0 px** (vs 4.5/4.0).

Honest diagnostic on live 516 yields **2.5/2.5 px** (identity p95 below
GAIA-00 reference 1.78 - current pass1 DAO stack, not gate survivors).
...
| Empty-sky audit (derived 2.5/2.5) | pass2 0.14%, seed 0.27% - **PASS** |

## Certificate

- `Archive/Drafts/draft_000516/platesolve/NoFilter_60_2/dao_gaia_calibration.json` - **PASS**
...
## Part 0 - SHA guard (unchanged GREEN)
...
`dev/tests/test_dao_gaia_calibration.py` - **6/6 PASS** (includes slow
validation gate integration on live 516).

## Files changed

- `src_py/dao_gaia_calibration.py` - A-fix 4 diagnostic mode + certificate audit fields
- `dev/tests/test_dao_gaia_calibration.py` - updated for diagnostic path
