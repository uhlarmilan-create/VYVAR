"""
Diagnostika ensemble normalizacie pre TIC v draft_198.
Porovna mag_inst(TIC) vs mag_inst(comps) frame-po-frame.
...
def main() -> None:
    ...
    for i, csv_p in enumerate(csvs[:82]):
        df = pd.read_csv(csv_p, low_memory=False, dtype={"catalog_id": str, "name": str})
    ...
    # Zarovnane polia (rovnaky frame pre vsetky veliciny)
    ...
    if n > 3:
        ...
        print("=== Airmass slopes (zarovnane frame-y) ===")
        ...
        if abs(diff) > 0.3:
            print("  [X] TIC a Ensemble maju rozdielny airmass response!")
            print("     Ensemble NEZRUSUJE airmass trend TIC.")
            print("     -> mozny vignetting / ina extinkcia na okraji vs comps")
        else:
            print("  [OK] TIC a Ensemble maju podobny airmass response")

    print()

    print("=== Porovnanie s lightcurve_344.csv (ulozena fazou 2A) ===")
    if lc_path.exists():
        ...
        if "delta_mag" in lc.columns:
            ...
            print(
                f"  delta_mag LC (vsetky riadky): min={dm_all.min():.4f}  max={dm_all.max():.4f}"
            )
            ...
            if len(delta_arr) > 0:
                print(
                    f"  Nasa delta (mean flux->mag, C01-C03 z CSV): min={delta_arr.min():.4f}  max={delta_arr.max():.4f}"
                )
                print(
                    "  Pozn.: Faza 2A pouziva mag_inst z read_flux + vahovany ensemble "
                    "(1/rms^2), nie aritmeticky mean dao_flux z raw CSV. "
                    "LC je pre catalog 344; mag_T vyssie je z ...280 (dao_flux v CSV pri susedovi)."
                )
                if len(dm_ok) > 0:
                    match = abs(float(dm_ok.min()) - float(delta_arr.min())) < 0.1
                    print(
                        f"  {'[OK] Zhodne (min vs normal)' if match else '[X] ROZDIEL (ocakavane - ina definicia ensemble)'}"
                    )
            else:
                print("  Nasa delta_mag: ziadne kompletne frame-y")
        else:
            print("  delta_mag stlpec chyba")
    else:
        print(f"  Chyba: {lc_path}")
