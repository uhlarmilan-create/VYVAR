#!/usr/bin/env python3
"""Resume chiandh draft 369: inject field center + re-preprocess + platesolve + photometry."""
...
def main() -> int:
    ...
    from night_run import _night_run_platesolve, _night_run_preprocess  # noqa: E402
    from pipeline import AstroPipeline  # noqa: E402

    sys.path.insert(0, str(_ROOT / "scripts"))
    import chiandh_night_run_bvr as nr  # noqa: E402

    nr._patch_field_center_resolve()
    orig = _patch_config()
    ...
    try:
        ...
        _night_run_preprocess(pending=job_ms, ap=ap, pipeline=pipeline, progress_cb=_prog_cb)

        from astropy.io import fits  # noqa: E402

        cal = draft_dir / "calibrated" / "lights"
        sample = next(cal.rglob("*.fits"))
        h = fits.getheader(sample)
        report["sample_header"] = {
    ...
    return 0 if report.get("platesolve") else 1
