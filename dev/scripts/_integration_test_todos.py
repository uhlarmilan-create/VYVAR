...
# Integracne testy TODO-1 az TODO-6 (1-18 automaticky; 11, 19-20 poznamky / staticke UI).
...
# field_density.json sa zapisuje do output_dir Fazy 0+1 (phase01), nie phase2a
...
def main() -> int:
    ...
    # --- TODO-2 VT REFRESH (1-3) - posledny beh (log moze byt appendovany) ---
    results[1] = "[VT REFRESH] x/y suradnice variable_targets.csv aktualizovane z MASTERSTAR WCS" in log_text
    ...
    if m_xy:
        ...
        margin = 80.0  # zvacseny efektivny frame + VT refresh mimo nominalneho cipu
        results[3] = (
            min(xa, xb) >= -margin
    ...
    pool_vals = _last_int_matches(r"\[GLOBAL COMP POOL\]\s+(\d+)\s+kandidatov", log_text)
    # 881 je typicke pri tomto drafte; rozsah sirsi ako povodnych 1000-1600
    if pool_vals:
        n_pool = pool_vals[-1]
    ...
    comp_lines = list(
        re.finditer(
            rf"Target {re.escape(target_id)}.*?:\s*(\d+)\s+porovnavaciek",
            log_text,
        )
    )
    ...
    tess_ok = "[TESS] preskocene" in log_text or "[AUTO] No candidates CSV found for crossmatch/TESS" in log_text
    # TESS/lightkurve: nehladat holy podretazec "mast" (MASTERSTAR ho obsahuje).
    ...
    # --- UI (19-20) - staticka kontrola kodu; skutocne spravanie = manualne v Streamlit ---
    ui_path = _ROOT / "ui_settings.py"
    ui_txt = ui_path.read_text(encoding="utf-8", errors="replace") if ui_path.is_file() else ""
    results[19] = "Gaia BP-RP primarny farebny filter" in ui_txt and "st.toggle(" in ui_txt
    results[20] = (
        "if use_bprp:" in ui_txt
        and "Tier1 |DeltaBP-RP| limit" in ui_txt
        and "Farebny filter - B-V (legacy)" in ui_txt
        and "Farebny filter - Gaia BP-RP (primarny)" in ui_txt
    )
