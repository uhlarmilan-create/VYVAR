...
st.append(P("Po kalibraci se na cely snimek fituje robustni polynomialni plocha radu 2 (6 clenu; preprocess_sky_surface_order=2) a odecita se CELA fitovana plocha vcetne konstantniho clenu (pedestal konvence dle T3 rozhodnuti; median snimku se posouva o fitovany pedestal, na referencnim snimku ~ -96 ADU). Odstranuje velkoplosny gradient (Mesic nizko, svitani) i zbytkove zakriveni po flatu; tvar i uroven pozadi jsou pak konzistentni pro DAO detekci a Labbe empiricke pozadi. Vyssi rad je ZAMERNE zakazan: polynom radu 3+ by zacal pozirat plosne objekty (mlhoviny) a lokalni struktury pozadi. Krok je soucasti anchoru draft_435 pro mono cestu. [b]OSC vetev (2026-07):[/b] pri nastavenem EQUIPMENTS.BAYERMASK se sky-surface na mozaice NIKDY nespousti (checkerboard by fit pokazil); kalibrace probiha na CFA, pak extrakce kanalu oneRGGB/R/G/B (plane split, prumer, bez interpolace) a sky-surface az na extrahovanych kanalech per obs-group. Viz osc_extract.py a OSC-01.", B))
