...
st.append(P("Misto pevneho prahu ostrosti se limit odvozuje z nocnich statistik (auto_fwhm_enabled=true): limit = median_FWHM_noci x k, kde k = auto_fwhm_k_factor (default 1.5) sevreny do [auto_fwhm_k_min=1.0, auto_fwhm_k_max=4.0]. Pevny strop qc_fwhm_limit=8.0 px plati, kdyz je auto rezim vypnuty. Dalsi meze: qc_max_hfr=5.0, qc_elong_limit=1.8 (protazeni = vedeni/vitr), qc_min_stars=10, volitelny strop qc_max_background_rms.", B))
...
st.append(par("qc_after_calibrate_enabled=true, qc_dao_detection_sigma=5.0, auto_fwhm_enabled=true (k=1.5, clamp 1.0..4.0), qc_fwhm_limit=8.0, qc_max_hfr=5.0, qc_elong_limit=1.8, qc_min_stars=10, qc_max_background_rms=null, frame_quality_gate_enabled=false (ratio_k=5.0, min_keep=10), frame_align_residual_gate_enabled=false (max_frac=0.25, min_keep=10)"))
...
st.append(P("Na MASTERSTARu bezi dvoupruchodova DAO detekce: bezny prah (masterstar_dao_threshold_sigma=2.1) a hlubsi druhy pruchod na stacku zarovnane serie s nizsim sumem - slaby konec katalogu se tak prohloubi bez zaplaveni falesnymi detekcemi. Strop poctu detekci je adaptivni k hustote pole (masterstar_detection_cap_adaptive=true, k=0.08, sevreno do 250..800). Detekce se krizove ztotoznuje s Gaia DR3 (match radius phase01_match_radius_arcsec=10.0), s VSX (zname promenne) a s lokalnim exoplanetovym katalogem (exoplanet_match_max_sep_arcsec=3.0). Kazda hvezda dostava stabilni identitu (Gaia source_id jako string), barvu BP-RP a vlajky: znama promenna, NSS (non-single star), exoplaneta, zona saturace/sumu (linear / noisy1-3 / saturated s priznaky is_saturated, is_noisy, is_usable dle peak_max_adu). Vystup: masterstars_full_match.csv. Gaia pozice se pred matchem posouvaji o vlastni pohyb k epose pozorovani (_apply_proper_motion z obs roku hlavicky).", B))
st.append(P("6.2 Hintovana cesta WCS", H2))
st.append(P("Kdyz FITS hlavicka nese pouzitelne zamereni (pointing_hint_from_header; RA/Dec z montaze), resi se WCS lokalne: detekce jasnych hvezd, parovani na Gaia vyrez, robustni fit TAN + SIP distorze (rad masterstar_platesolve_sip_min_order=3 az max_order=4, adaptivni ridge regularizace dle poctu shod). Prijeti reseni je statisticke (masterstar_accept_mode='odds' - odds-ratio test) s tvrdymi zavorami: obnoveni >= masterstar_catalog_recovery_min=0.65 podilu katalogovych hvezd, absolutni dno masterstar_min_matched_floor=40 shod, RMS stredu <= masterstar_centre_rms_max_px=1.2, benigni pomer distorze okraj/stred <= 3.2.", B))
...
st.append(par("masterstar_dao_threshold_sigma=2.1, masterstar_best_of_n=10, masterstar_accept_mode='odds', masterstar_catalog_recovery_min=0.65, masterstar_min_matched_floor=40, masterstar_centre_rms_max_px=1.2, masterstar_platesolve_sip_min/max_order=3/4, sibling recovery (stack 10, matched>=40, quadrants>=3, rms<=2.0), detection cap adaptive (k=0.08, 250..800), blind_* (budget 80, verify frac 0.15, matches 12, tol 2.5 px, top 15, early 30), verify_mag_limit=14.0, phase01_match_radius_arcsec=10.0, exoplanet_match_max_sep_arcsec=3.0, vsx_variable_targets_mag_limit=14.5"))
...
st.append(par("sips_dao_fwhm_px=2.5, sips_dao_threshold_sigma=3.5 (SIPS-styl preset), qc_dao_detection_sigma=5.0, masterstar_dao_threshold_sigma=2.1"))

# ============================================================ CH 9
st.append(P("9. Faze 0 - priprava cilu (active targets)", H1))
st.append(P("Cile mereni vznikaji tremi cestami: [b](1)[/b] automaticky ze VSX - zname promenne v poli do vsx_variable_targets_mag_limit=14.5 mag; nevyresene VSX hvezdy se dohledavaji primym Gaia DR3 lookupem (VSX -> Gaia fallback), takze podil neprirazenych se blizi nule; [b](2)[/b] automaticky z lokalniho exoplanetoveho katalogu (hostitele tranzitu; match 3.0 arcsec); [b](3)[/b] rucne zadane cile uzivatele v UI. Kazdy cil se pinuje na Gaia source_id - identita je katalogova, ne pixelova, takze prezije zmenu pole i sestavy.", B))
...
st.append(tab([
 ...
 ["Barva (primarni!)","|Delta(BP-RP)| <= comp_max_delta_bprp=0.79; phase01_use_bprp_primary=true - barva je first-order kriterium: u NoFilter/sirokopasmovych dat neexistuje filtr, ktery by barevny clen vyrusil, takze barevna shoda kompu je HLAVNI obrana proti extinkci 2. radu"],
...
st.append(tab([
 ...
 ["COG aperturni korekce","cog_aperture_correction_enabled=false - riziko mixed-frame na castecne uspesne noci (11.5)."],
], [42,126]))
...
st.append(P("[b]build_epsf_grid_model:[/b] pole se rozdeli na bunky (psf_spatial_grid='3x3', minimum psf_spatial_min_stars_per_cell=25 hvezd na bunku) a v kazde se stavi LOKALNI ImagePSF - mrizka modelu zachycuje prostorovou promennost PSF pres pole (koma v rozich, naklon ohniskove roviny). Mereni pak interpoluje model pro konkretni pozici (interp_gridded_epsf_array). Prepinac psf_spatial_enabled=false; psf_spatial_order=0 znamena konstantni PSF.", B))
...
st.append(tab([
 ...
 ["FILT","AAVSO kod z band_classify (NoFilter -> CV pres aavso_filter_map; neznamy filtr -> #WARNING, nikdy tiche CV)"],
...
st.append(P("CSV/TXT format pro ceskou sekci; obsahuje # Trust: ... hlavicku. Smerovani vysledku: zakrytove dvojhvezdy -> VarAstro (LC), pulzujici a ostatni -> AAVSO. Citacni hlavicky exportu emituje citations.py PODMINENE dle metod, ktere skutecne bezely (CITATIONS.bib je kanonicky zdroj).", B))
st.append(fn("report_methods.py: aavso_export_path a export cesta; pdf_report.py, photometry_report.py (reportlab); band_classify.py: guess_aavso_code_from_obs_group; hrd_analysis.py: build_hrd_dataframe; hrd_colorfield.py, hrd_enrich.py; citations.py; dev/scripts + scripts: validate_aavso_export.py; time_utils.py (HJD/BJD)"))
...
for r in [
 "Astropy Collaboration 2013/2018/2022 - astropy (7.2.0).",
 "Bradley L. et al. - photutils (2.3.0; DAOStarFinder, aperture, EPSFBuilder, PSFPhotometry).",
...
st.append(tab([
 ...
 ["vsx_variable_targets_mag_limit (14.5)","dle hloubky rigu (viz G_lim_90 v reportu); planovana automatika z limitni magnitudy"],
...
st.append(tab([
 ...
 ["docs/*_SPEC.md (CAL_DIAG, K2_DESIGN, SIGMA_FLOOR, SPARSE_TRUST, SIGMA_BUDGET...)","technicke specifikace jednotlivych subsystemu"],
], [70,98]))
