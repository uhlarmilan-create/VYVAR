...
4. **UI parity.** The same parameter is surfaced in the Settings UI (label, help, range), and
   recorded in `VYVAR_PARAMS.md`. *A parameter that exists only in config is not done.*
5. **Docs + citation.** Update `VYVAR_STATE/ROADMAP/DECISIONS/JOURNAL` as appropriate; close
