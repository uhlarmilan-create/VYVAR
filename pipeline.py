...
def _calibrate_one_light_apply_masters_in_ram(
    *,
    ...
    cal_diag_gate_enabled: bool = False,
    ...
    if md_data is None and master_dark_path is not None and master_dark_path.exists():
        from cal_diag import convention_to_dark_mode

        _drm = "sum"
        if cal_diag_gate_enabled and cal_diag_gate_result is not None:
    ...
    apply_cal_diag_headers(hdr, cal_diag_gate_result, gate_enabled=cal_diag_gate_enabled)
...
def _calibrate_one_light_disk(
    *,
    ...
    data, hdr, used_dark, used_flat = _calibrate_one_light_apply_masters_in_ram(
        ...
        cal_diag_gate_enabled=cal_diag_gate_enabled,
...
_cal_batch_cal_diag_enabled: bool = False


def _init_calibrate_batch_worker(
    initargs: tuple[str | None, int | None, dict[str, Any] | None, bool],
) -> None:
    ...
    global _cal_batch_native_binning, _cal_batch_cal_diag, _cal_batch_cal_diag_enabled
    _md_s, native_b, cal_diag_blob, gate_on = initargs
    ...
    _cal_batch_cal_diag_enabled = bool(gate_on)
    if native_b is None:
...
def _calibrate_batch_process_one(
    ...
    try:
        ...
        if _cal_batch_cal_diag_enabled and _cal_batch_cal_diag is not None:
            if is_obs_group_aborted(_cal_batch_cal_diag, _ok):
                if dst_p.exists():
        ...
        if md_use is not None and md_use.is_file() and _cal_batch_cal_diag_enabled:
            ...
            md_np = dark_np_for_cal_diag(
                ...
                gate_result=gr,
                gate_enabled=_cal_batch_cal_diag_enabled,
            )
        ...
        _ud, _uf, qc_sum, _cf, perf10_qc = _calibrate_one_light_disk(
            ...
            cal_diag_gate_enabled=_cal_batch_cal_diag_enabled,
            cal_diag_gate_result=gr,
        )
        return {
            "src": src_s,
...
def _passthrough_lights_to_calibrated(
    *,
    ...
    passthrough_cal_diag: bool = False,
) -> dict[str, Any]:
    ...
    for i, src in enumerate(files, start=1):
        ...
        try:
            ...
            passthrough_cal_diag_headers(hdr, gate_enabled=bool(passthrough_cal_diag))
            fits.writeto(dst, _as_fits_float32_image(data), header=hdr, overwrite=True)
            stats["processed"] += 1
            stats["copied_only"] += 1
            if db_pt is not None:
    ...
    *,
    ...
    if not _has_dark_any:
        ...
        return _passthrough_lights_to_calibrated(
            ...
            draft_id=draft_id,
            observation_id=observation_id,
            passthrough_cal_diag=bool(cal_diag_config_from_app(cfg).get("enabled", True)),
        )
    ...
    if _vyvar_calibrate_multiprocessing_enabled() and nw > 1 and total > 1:
        ...
        try:
            with ProcessPoolExecutor(
                ...
                initargs=(md_init_str, _native_b, cal_diag_worker_blob, gate_on),
            ) as ex:
                future_map = {
...
class AstroPipeline:
    ...
    def calibrate_batch(
        ...
        *,
        ...
        gate_cfg = cal_diag_config_from_app(self.config)
        gate_on = bool(gate_cfg.get("enabled", True))
        cal_diag_session = CalDiagSession()
        db_cal = _db_for_calibration_tasks(qc_pack)

        if gate_on and light_paths:
            cal_diag_session = run_cal_diag_pregate(
                light_paths,
                obs_group_key_from_path=_obs_group_key_from_light_path,
        ...
        if nw <= 1:
            ...
            for i, it in enumerate(items):
                ...
                try:
                    _ok = _obs_group_key_from_light_path(src_p)
                    if gate_on and is_obs_group_aborted(cal_diag_session, _ok):
                        if dst_p.exists():
                            dst_p.unlink(missing_ok=True)
                        rows.append(
                            {
            ...
            with ProcessPoolExecutor(
                ...
                mp_context=ctx,
                initializer=_init_calibrate_batch_worker,
                initargs=(md_init, _native_b, cal_diag_worker_blob, gate_on),
            ) as ex:
