...
| **T2 coverage (81″ unmatched DET)** | **CONFIRMED** (spurious/unmatched DAO; not stale coords; not radius-fixable) |
...
1. **Isotropic, not rigid shift:** vector_mean ≈ 0.1″ vs mean|Δ| ≈ 85″ for unmatched DET_*; corr(|Δ|, r) ≈ 0.003; recompute-invariance → unmatched DET sky coords are **fine**; 81″ median = no nearby catalog row, not WCS staleness.
2. **Two matched populations:** (a) ms-gaia ≈ 0″ rows with detection-time Gaia coords (e.g. FW CVn stored); (b) 164 rows where **stored ≈ WCS(x,y)** but **both** are 4–90″ from assigned `catalog_id` Gaia position — **catalog_id ↔ pixel sky mismatch**, not stale-metadata-only.

---

## T1 — Pixel-space identity test

**Population:** 184 rows with vt↔ms sep > 2″ (179 from v3 + 5 active-only without vt row).

| Class | Count | Meaning |
|-------|------:|---------|
| **MISASSIGNED-ID** | **164** | sep(WCS(x,y), Gaia[catalog_id]) > 3.9″ |
| CONSISTENT | 20 | sep(WCS(x,y), Gaia) ≤ 3.9″ |
| STALE-COORDS | 0 | — |

**Subclass (184 violations):**
| Subclass | Count | Interpretation |
|----------|------:|----------------|
| wcs_stored ≤ 1″ | 177 | Stored ra/dec **tracks** final WCS(x,y) — **not** initial-WCS staleness |
| stored_gaia ≤ 1″ | 3 | Metadata forced to Gaia; pixels elsewhere (FW CVn, HAT-188-0000203, …) |
...
|--------|-------|--------------:|-------------:|-----------:|----------------|
| FY CVn | MISASSIGNED | 11.8″ | ~12.0″ | 12.9″ | EW, lc_rms=10.4%, dilution=1.0 |
| FZ CVn | MISASSIGNED | 12.8″ | ~12.7″ | 13.5″ | EW, lc_rms=19.0%, dilution=1.0 |
| CSS_J134925.3+393524 | MISASSIGNED | 18.2″ | ~18.3″ | 19.3″ | RRAB, lc_rms=23.0% |
| CSS_J140918.7+423422 | MISASSIGNED | 13.9″ | — | 13.8″ | RRAB, lc_rms=22.9% |
| NSVS 5096293 | MISASSIGNED | 6.5″ | ~7.8″ | 7.7″ | EB, lc_rms=17.6% |
| RX CVn | MISASSIGNED | 8.4″ | ~8.5″ | 9.0″ | RRAB, lc_rms=6.6% |
| R CVn | MISASSIGNED | 13.9″ | — | 12.9″ | (excluded from active) |

**LC corroboration:** Active targets show **expected VSX types** and plausible variability amplitude — **does not** resolve MISASSIGNED-ID under the task gate (pixel position is 6–19″ from VSX **and** Gaia for the assigned cid). Milan should visually overlay x/y on MASTERSTAR vs VSX/Gaia before accepting science.

**MISASSIGNED-ID explicit list:** all 164 rows in `tmp/f428_coord_forensics_v4.txt` § MISASSIGNED-ID explicit list.

---

## T2 — Gaia coverage vs frame (81″ unmatched DET)

| Item | Value |
|------|-------|
| Global SQLite DB | 211.7M rows; bbox essentially all-sky (not field-scoped) |
| Field catalog cone | r ≈ 13.6°, center (209.49°, 41.16°); export cap 100k, G ≤ 15.26 |
| Sources in frame footprint | **4379** in `field_catalog_cone.csv` |
| Unmatched DET_* | 2724 |
| NN p50 (sqlite field slice) | 81.053″ |
| NN p50 (field_catalog in frame) | **131.1″** |
| Within 20″ of field_catalog | **11 / 2724** |

**COVERAGE VERDICT (81″ population): CONFIRMED** — unmatched DET are **spurious/faint DAO detections without catalog association** in the loaded cone; **not** fixable by match-radius tuning. Actionable item remains **deeper/wider field DB** (GAIA-DR4 build — Milan decision; out of scope).

**Separate from T1:** the 164 MISASSIGNED matched rows are **not** the 81″ unmatched population.

---

## T3 — Magnitude-consistency backstop

- Reference fit: 246 rows with ms-gaia ≤ 0.5″
...
| **GS11 dilution cone query** | `dilution.py`, `photometry_core.py` ~8482 | **ra/dec** | **Low** for 177/184 (stored ≈ WCS xy); **Yes** for 3 stored_gaia≤1″ | FY/FZ/RX dilution_factor=**1.0** in photometry_summary |
...
2. **Visual overlay:** MASTERSTAR + x/y markers vs Gaia/VSX for FY CVn, FZ CVn, RX CVn (6–13″ offsets).
...
4. **81″ unmatched DET:** accept as spurious-detection population; radius decision stays OPEN; field DB depth is separate track.
